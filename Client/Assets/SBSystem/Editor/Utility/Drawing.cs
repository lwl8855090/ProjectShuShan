...
public class Drawing
{
    ...
    public static Texture2D rectTex = null;
    ...
    public static void DrawRect(Rect rec, Color color)
    {
        Color savedColor = GUI.color;
        if (!rectTex)
        {
            rectTex = new Texture2D(1, 1, TextureFormat.ARGB32, true);
            rectTex.SetPixel(0, 1, Color.white);
            rectTex.Apply();
        }

        GUI.color = color;
        GUI.DrawTexture(rec, rectTex);
        GUI.color = savedColor;
    }
